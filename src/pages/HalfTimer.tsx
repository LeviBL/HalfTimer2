...
const HalfTimer: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gray-50 p-4 pt-20 text-gray-800 relative">
      ...
      {loading ? (
        <div className="grid grid-cols-1 min-[740px]:grid-cols-2 gap-6 w-full max-w-[720px] mx-auto">
          {[...Array(6)].map((_, i) => (
        ...
        <div className={cn(
          ...
          games.length === 1 ? "flex justify-center" : "grid grid-cols-1 min-[740px]:grid-cols-2 gap-6"
        )}>
          {games.length > 0 ? (
            games.map((game) => (
              <GameCard
