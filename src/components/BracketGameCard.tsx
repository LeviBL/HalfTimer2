...
const BracketGameCard: React.FC<BracketGameCardProps> = ({ game, onClick }) => {
  ...
  return (
    <Card 
      ...
      className={cn(
        "w-48 p-2 cursor-pointer hover:shadow-md transition-all border-2",
      ...
      <div className="space-y-1">
        ...
        <div className="flex justify-between items-center text-xs">
          <div className="flex items-center gap-1 overflow-hidden">
            <span className="text-gray-500 font-bold w-4">({game.competitors.away.seed || "?"})</span>
            <img src={game.competitors.away.logo} className="w-4 h-4 object-contain" alt="" />
            <span className="truncate font-medium">{game.competitors.away.displayName}</span>
          </div>
          <span className="font-bold">{game.competitors.away.score}</span>
        </div>

        {/* Home Team */}
        <div className="flex justify-between items-center text-xs">
          <div className="flex items-center gap-1 overflow-hidden">
            <span className="text-gray-500 font-bold w-4">({game.competitors.home.seed || "?"})</span>
            <img src={game.competitors.home.logo} className="w-4 h-4 object-contain" alt="" />
            <span className="truncate font-medium">{game.competitors.home.displayName}</span>
          </div>
          <span className="font-bold">{game.competitors.home.score}</span>
        </div>

        {/* Status */}
        <div className="pt-1 border-t border-gray-100 text-[10px] text-center font-bold">
          {isHalftime ? (
            <span className="text-amber-600">Halftime: {halftimeRemaining !== null ? formatCountdown(halftimeRemaining) : "..."}</span>
          ) : isScheduled ? (
            <span className="text-gray-500">{new Date(game.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
          ) : isFinal ? (
