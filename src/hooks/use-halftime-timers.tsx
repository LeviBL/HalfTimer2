...
interface HalftimeContextType {
  ...
  clearHalftimeStartTime: (gameId: string) => Promise<void>;
  isLoading: boolean;
...
export const HalftimeTimersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // 2. Universal realtime subscription
  ...
  const getHalftimeStartTime = useCallback(
  ...
  // Synchronize a new halftime start time ONLY if one does not already exist
  const ensureHalftimeTimer = useCallback(
    async (gameId: string): Promise<number | undefined> => {
  ...
  const clearHalftimeStartTime = useCallback(async (gameId: string) => {
    ...
    try {
      await supabase.from("halftime_timers").delete().eq("game_id", gameId);
    } catch (err) {
